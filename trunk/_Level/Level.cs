...
//    GameObject[] m_lCheckpoints;
    ...
    void OnGUI()
    {
        if (Application.loadedLevel > 0 && Application.loadedLevel <= Game.Instance.NumLevelsInWorld)
            GUI.Label(new Rect(170, 5, 200, 40), "World " + m_Game.CurrWorldNum.ToString() + ", Level " + m_Game.CurrLevelNum.ToString()/*, m_GUIStyle*/);
        else if (Application.loadedLevelName == "PracticeLevel")
            GUI.Label(new Rect(170, 5, 200, 40), "Practice"/*, m_GUIStyle*/);
    }
